...
(function($) {
	SrkaImageWidget.updateSizeDropdownOptions = function( field, sizes ) {
		...
		if ( sizes ) {
			$.each( sizes, function( key, size ) {
				...
				if ( key in SrkaImageWidget.imageSizeNames ) {
					name = SrkaImageWidget.imageSizeNames[ key ];
				}
		...
		if ( ! options ) {
			name = SrkaImageWidget.imageSizeNames['full'] || SrkaImageWidget.fullSizeLabel;
	...
	mediaControl = {
		...
		frame: function() {
			...
			this._frame = wp.media({
				title: $control.data('title') || SrkaImageWidget.frameTitle,
				library: {
					type: $control.data('media-type') || 'image'
				},
				button: {
					text: $control.data('update-text') || SrkaImageWidget.frameUpdateText
				},
		...
		select: function() {
			...
			$control.trigger( 'selectionChange.srkaimagewidget', [ selection ] );
		},
		...
		init: function() {
			$('#wpbody').on('click', '.srka-image-widget-control-choose', function(e) {
				var targetSelector;
				
				e.preventDefault();
				
				$control = $(this).closest('.srka-image-widget-control');
				
				targetSelector = $control.data('target') || '.srka-image-widget-control-target';
