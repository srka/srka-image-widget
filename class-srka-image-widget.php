 ...
 * @since 3.0.0
 */
class Srka_Image_Widget extends WP_Widget {
	/**
	...
	function __construct( $id_base = false, $name = false, $widget_options = array(), $control_options = array() ) {
		$id_base = ( $id_base ) ? $id_base : 'srkaimage'; // Legacy ID.
		$name = ( $name ) ? $name : __( 'Image', 'srka-image-widget' );

		$widget_options = wp_parse_args( $widget_options, array(
			'classname'   => 'widget_srkaimage', // Legacy class name.
			'description' => __( 'Display an image', 'srka-image-widget' ),
		) );
	...
	function widget( $args, $instance ) {
		$cache = (array) wp_cache_get( 'srka_image_widget', 'widget' );
		...
		wp_cache_set( 'srka_image_widget', array_filter( $cache ), 'widget' );
	}
	...
	function render( $args, $instance ) {
		...
		$output = $args['before_widget'];
			...
			if ( $inside = apply_filters( 'srka_image_widget_output', '', $args, $instance, $this->id_base ) ) {
				$output .= $inside;
			} else {
				...
				// Add the image.
				if ( ! empty( $instance['image_id'] ) ) {
					$image_size = ( ! empty( $instance['image_size'] ) ) ? $instance['image_size'] : apply_filters( 'srka_image_widget_output_default_size', 'medium', $this->id_base );

					$output .= sprintf( '<p class="srka-image">%s%s%s</p>',
						$instance['img_link_open'],
	...
	function form( $instance ) {
		...
		$button_class = array( 'button', 'button-hero', 'srka-image-widget-control-choose' );
		$image_id = $instance['image_id'];

		// The order of fields can be modified, new fields can be registered, or existing fields can be removed here.
		$fields = (array) apply_filters( 'srka_image_widget_fields', $this->form_fields(), $this->id_base );
		?>

		<div class="srka-image-widget-form">

			<?php do_action( 'srka_image_widget_form_before', $instance, $this->id_base ); ?>

			<p>
				<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'srka-image-widget' ); ?></label>
				<input type="text" name="<?php echo $this->get_field_name( 'title' ); ?>" id="<?php echo $this->get_field_id( 'title' ); ?>" value="<?php echo esc_attr( $instance['title'] ); ?>" class="widefat">
			</p>

			<?php if ( ! is_srka_image_widget_legacy() ) : ?>
				<p class="srka-image-widget-control<?php echo ( $image_id ) ? ' has-image' : ''; ?>"
					data-title="<?php esc_attr_e( 'Choose an Image for the Widget', 'srka-image-widget' ); ?>"
					data-update-text="<?php esc_attr_e( 'Update Image', 'srka-image-widget' ); ?>"
					...
					?>
					<input type="hidden" name="<?php echo $this->get_field_name( 'image_id' ); ?>" id="<?php echo $this->get_field_id( 'image_id' ); ?>" value="<?php echo $image_id; ?>" class="image-id srka-image-widget-control-target">
					<a href="#" class="<?php echo join( ' ', $button_class ); ?>"><?php _e( 'Choose an Image', 'srka-image-widget' ); ?></a>
				</p>
			<?php endif; ?>

			<?php if ( is_srka_image_widget_legacy() || ! empty( $instance['image'] ) ) : ?>
				<div class="srka-image-widget-legacy-fields">
					<?php if ( ! is_srka_image_widget_legacy() ) : ?>
						<p>
							<em><?php _e( 'These fields are here to maintain your data from an earlier version.', 'srka-image-widget' ); ?></em>
						</p>
						<p>
							<em><?php _e( 'Select an image, then clear these values, and they will disappear when you save the widget.', 'srka-image-widget' ); ?></em>
						</p>
					<?php endif; ?>

					<p>
						<label for="<?php echo $this->get_field_id( 'image' ); ?>"><?php _e( 'Image URL:', 'srka-image-widget' ); ?></label>
					...
					<p>
						<label for="<?php echo $this->get_field_id( 'alt' ); ?>"><?php _e( 'Alternate Text:', 'srka-image-widget' ); ?></label>
				...
				foreach ( $fields as $field ) {
					switch ( $field ) {
						case 'image_size' :
							...
							?>
							<p>
								<label for="<?php echo $this->get_field_id( 'image_size' ); ?>"><?php _e( 'Size:', 'srka-image-widget' ); ?></label>
                        ...
                        case 'img_link' :
                            ...
                            <p style="margin-bottom: 0.25em">
                                <label for="<?php echo $this->get_field_id( 'img_link' ); ?>"><?php _e( 'Image Link:', 'srka-image-widget' ); ?></label>
                            ...
                            <p style="padding-left: 2px">
                                <label for="<?php echo $this->get_field_id( 'img_new_window' ); ?>">
                                    ...
                                    <?php _e( 'Open in new window?', 'srka-image-widget' ); ?>
                                </label>
                            </p>
                            <?php
							break;
						...
						case 'link' :
							?>
							<p style="margin-bottom: 0.25em">
								<label for="<?php echo $this->get_field_id( 'link' ); ?>"><?php _e( 'Link:', 'srka-image-widget' ); ?></label>
						...
						case 'link_text' :
							?>
							<p>
								<label for="<?php echo $this->get_field_id( 'link_text' ); ?>"><?php _e( 'Link Text:', 'srka-image-widget' ); ?></label>
						...
						case 'text' :
							?>
							<p>
								<label for="<?php echo $this->get_field_id( 'text' ); ?>"><?php _e( 'Text:', 'srka-image-widget' ); ?></label>
						...
						default :
							...
							do_action( 'srka_image_widget_field-' . sanitize_key( $field ), $instance, $this );
					}
				}
			}

			do_action( 'srka_image_widget_form_after', $instance, $this->id_base );
			?>
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance = apply_filters( 'srka_image_widget_instance', $instance, $new_instance, $old_instance, $this->id_base );
	...
	function get_image_sizes( $image_id ) {
		$sizes = array( 'full' => __( 'Full Size', 'srka-image-widget' ) );

		$imagedata = wp_get_attachment_metadata( $image_id );
		if ( isset( $imagedata['sizes'] ) ) {
			$size_names = Srka_Image_Widget_Loader::get_image_size_names();
	...
	function flush_group_cache( $post_id = null ) {
		...
		wp_cache_delete( 'srka_image_widget', 'widget' );
	}
}
